...
public class UserController {
    ...
    //This method also validates Password strength if it matches the condition, "password entered by the user must contain
    // at least 1 alphabet (a-z or A-Z), 1 number (0-9) and 1 special character" else it adds an error attribute to model and redirect user back to registration page
    @RequestMapping(value = "users/registration", method = RequestMethod.POST)
    public String registerUser(User user, Model model) {
        String error = "Password must contain atleast 1 alphabet, 1 number & 1 special character";
        boolean test = userService.validPassword(user.getPassword());
        if(!test) {
            model.addAttribute("User",user);
